...
class BPlus_tree final : compare {
public:
...
private:
    ...
    enum class bptree_page_type : std::uint8_t {
        term,
        middle,
        free
    };

    struct bptree_file_header {
        std::uint64_t _magic{};
    ...
    struct bptree_node_header {
        bptree_page_type _type{bptree_page_type::term};
    ...
    static constexpr size_t maximum_keys_in_node = 2 * t;
    ...
    static constexpr std::uint64_t disk_magic = 0x42505452454531ULL;
    static constexpr std::uint32_t disk_version = 1;
    static constexpr std::uint64_t disk_variable_page_layout = 1;

    static constexpr std::size_t disk_term_header_size =
            sizeof(bptree_page_type) + sizeof(std::uint16_t) + 3 * sizeof(storage::page_id_t);



    bool compare_keys(const TKey &lhs, const TKey &rhs) const { return compare::operator()(lhs, rhs); }
    storage::Pager _pager;
    bptree_file_header _disk_header{};
    bool _copy_on_write_enabled{true};


public:

    explicit BPlus_tree(const std::filesystem::path &path, const compare &cmp = compare());
    BPlus_tree(const std::filesystem::path &path, const BPlus_tree &other, const compare &cmp = compare());



    BPlus_tree(const BPlus_tree &other) = delete;
    BPlus_tree(BPlus_tree &&other) noexcept;
    BPlus_tree &operator=(const BPlus_tree &other) = delete;
    BPlus_tree &operator=(BPlus_tree &&other) noexcept;
    ~BPlus_tree() noexcept = default;
    ...
    public:
        ...
        explicit bptree_iterator() noexcept = default;

    private:
        void load_disk_term(storage::page_id_t page_id);
        explicit bptree_iterator(const BPlus_tree *owner, storage::page_id_t page_id, size_t index,
                                 bptree_node_term &&term);
    };
    ...
    bptree_iterator begin() const {
        static_assert(std::is_copy_constructible_v<TKey> && std::is_copy_constructible_v<TValue>,
                      "disk-backed BPlus_tree iterators require copy-constructible key and value types");
        return _disk_header._size == 0 || _disk_header._first_term == storage::invalid_page_id
                   ? end()
                   : bptree_iterator(this, _disk_header._first_term, 0, read_term(_disk_header._first_term));
    }

    bptree_iterator end() const { return bptree_iterator(); }
    ...
    bptree_iterator find(const TKey &key) const {
        auto it = bound(key);
        return it != end() && keys_equal(key, it->first) ? it : end();
    }

    bptree_iterator lower_bound(const TKey &key) const { return bound(key, false); }
    bptree_iterator upper_bound(const TKey &key) const { return bound(key, true); }
...
private:
    ...
    BPlus_tree(const std::filesystem::path &path, const compare &cmp, bool is_new_file, bool copy_on_write = true);

    template<typename Fn>
    void run_copy_on_write(Fn &&fn);

    bptree_iterator bound(const TKey &key, bool strict = false) const;
    bool keys_equal(const TKey &a, const TKey &b) const noexcept { return !compare_keys(a, b) && !compare_keys(b, a); }

    static bptree_node_term make_term(const storage::page_id_t self, const storage::page_id_t parent,
                                      const storage::page_id_t next = storage::invalid_page_id) {
        return {._header = {bptree_page_type::term, 0, self, parent}, ._next = next};
    }

    static bptree_node_middle make_middle(const storage::page_id_t self, const storage::page_id_t parent) {
        return {._header = {bptree_page_type::middle, 0, self, parent}};
    }

    std::tuple<storage::page_id_t, bptree_node_term, size_t> get_disk_term_and_index(const TKey &key) const;
    static void validate_disk_payload(const tree_data_type &data);

    void distribute_term_pair(bptree_node_term &left, bptree_node_term &right) const;
    void distribute_middle_pair(bptree_node_middle &left, bptree_node_middle &right);
    void set_children_parent(const bptree_node_middle &node);
    ...
    bptree_page_type read_page_type(storage::page_id_t page_id) const;
    bptree_node_term read_term(storage::page_id_t page_id) const;
    void write_term(storage::page_id_t page_id, const bptree_node_term &node);
    bptree_node_middle read_middle(storage::page_id_t page_id) const;
    void write_middle(storage::page_id_t page_id, const bptree_node_middle &node);
    ...
    void rebuild_middle_keys(bptree_node_middle &node);

    void wrebuilt_middle(bptree_node_middle &node) {
        rebuild_middle_keys(node);
        write_middle(node._header._self, node);
    }

    void rebalance_term(storage::page_id_t term_page, bptree_node_term &&term);
    void rebalance_middle(storage::page_id_t middle_page, bptree_node_middle &&node);

    void split_root_term(bptree_node_term &&term);
    void split_term_node(storage::page_id_t term_page, bptree_node_term &&term);
    void split_root_middle(bptree_node_middle &&root);
    void split_middle_node(storage::page_id_t middle_page, bptree_node_middle &&node);

};


template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::BPlus_tree(const std::filesystem::path &path, const compare &cmp) : BPlus_tree(
        path, cmp, is_new_disk_file(path), true) {
}

template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::BPlus_tree(const std::filesystem::path &path, const compare &cmp,
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::BPlus_tree(const std::filesystem::path &path, const BPlus_tree &other,
                                                 const compare &cmp) : BPlus_tree(path, cmp, is_new_disk_file(path)) {
    if (std::filesystem::exists(path) && std::filesystem::equivalent(path, other._pager.path())) {
        throw std::invalid_argument("BPlus_tree destination path must differ from source path");
    }
    clear();
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare,
           t>::BPlus_tree(BPlus_tree &&other) noexcept : compare(std::move(static_cast<compare &>(other))),
                                                         _pager(std::move(other._pager)),
                                                         _disk_header(std::exchange(other._disk_header, bptree_file_header{})),
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t> &BPlus_tree<TKey, TValue, compare, t>::operator=(BPlus_tree &&other) noexcept {
    if (this == &other) return *this;
    BPlus_tree tmp(std::move(other));
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
template<typename Fn>
void BPlus_tree<TKey, TValue, compare, t>::run_copy_on_write(Fn &&fn) {
    ...
    try {
        {
            BPlus_tree shadow(temp_path, static_cast<const compare &>(*this), false, false);
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_iterator::bptree_iterator(const BPlus_tree *owner,
                                                                       const storage::page_id_t page_id,
                                                                       const size_t index,
                                                                       bptree_node_term &&term) : _index(index),
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::bptree_iterator::load_disk_term(const storage::page_id_t page_id) {
    static_assert(std::is_copy_constructible_v<TKey> && std::is_copy_constructible_v<TValue>,
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_iterator::reference BPlus_tree<TKey, TValue, compare,
                                                                            t>::bptree_iterator::operator*() const {
    return _disk_data[_index];
}

template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_iterator::pointer BPlus_tree<TKey, TValue, compare,
                                                                          t>::bptree_iterator::operator->() const {
    return &_disk_data[_index];
}

template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_iterator &BPlus_tree<TKey, TValue, compare, t>::bptree_iterator::operator
++() {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_iterator BPlus_tree<TKey, TValue, compare, t>::
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
bool BPlus_tree<TKey, TValue, compare, t>::bptree_iterator::operator==(const self &other) const noexcept {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::clear() {
    if (_copy_on_write_enabled) {
        run_copy_on_write([](BPlus_tree &shadow) { shadow.clear(); });
        return;
    }
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
std::pair<typename BPlus_tree<TKey, TValue, compare, t>::bptree_iterator, bool> BPlus_tree<TKey, TValue, compare,
                                                                                            t>::insert(tree_data_type data) {
    static_assert(std::is_copy_constructible_v<TKey> && std::is_copy_constructible_v<TValue>,
                  "disk-backed BPlus_tree insertion requires copy-constructible key and value types");
    ...
    if (_copy_on_write_enabled) {
        ...
        run_copy_on_write([payload = std::move(data)](BPlus_tree &shadow) mutable {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_iterator BPlus_tree<TKey, TValue, compare, t>::insert_or_assign(
        tree_data_type data) {
    static_assert(std::is_move_assignable_v<TValue>,
                  "disk-backed BPlus_tree insert_or_assign requires move-assignable value type");
    validate_disk_payload(data);
    const auto k_copy = data.first;
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_iterator BPlus_tree<TKey, TValue, compare, t>::erase(
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_iterator BPlus_tree<TKey, TValue, compare, t>::erase(const TKey &key) {
    static_assert(std::is_copy_constructible_v<TKey> && std::is_copy_constructible_v<TValue>,
                  "disk-backed BPlus_tree erase requires copy-constructible key and value types");
    ...
    if (_copy_on_write_enabled) {
        run_copy_on_write([&key](BPlus_tree &shadow) {
            shadow.erase(key);
        });
    ...
    if (read_page_type(_disk_header._root_page) == bptree_page_type::middle) {
        ...
        while (root._pointers.size() == 1) {
            ...
            if (read_page_type(_disk_header._root_page) == bptree_page_type::term) {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_iterator
BPlus_tree<TKey, TValue, compare, t>::bound(const TKey &key, const bool strict) const {
    if (_disk_header._size == 0) return end();
    auto [term_page, term, index] = get_disk_term_and_index(key);
    ...
    if (index < term.data.size()) return bptree_iterator(this, term_page, index, std::move(term));

    if (term._next != storage::invalid_page_id) {
        const auto next_page = term._next;
        return bptree_iterator(this, next_page, 0, read_term(next_page));
    }
    return end();
}

template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
std::tuple<storage::page_id_t, typename BPlus_tree<TKey, TValue, compare, t>::bptree_node_term, size_t>
BPlus_tree<TKey, TValue, compare, t>::get_disk_term_and_index(const TKey &key) const {
    auto term_page = _disk_header._root_page;
    while (read_page_type(term_page) == bptree_page_type::middle) {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::validate_disk_payload(const tree_data_type &data) {
    ...
    if (const auto value = storage::Serializer<TValue>::to_bytes(data.second);
        ...
        throw std::runtime_error("Serialized BPlus_tree key/value pair is too large for one disk page");
    }
}
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::distribute_term_pair(bptree_node_term &left,
                                                                bptree_node_term &right) const {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::distribute_middle_pair(
        bptree_node_middle &left,
        bptree_node_middle &right) {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::set_children_parent(const bptree_node_middle &node) {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::load_header() {
    ...
    if (_disk_header._magic != disk_magic || _disk_header._version != disk_version) {
        throw std::runtime_error("Corrupted BPlus_tree disk header");
    }
}

template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::validate_header() const {
    const auto file_size = std::filesystem::file_size(_pager.path());
    if (file_size % storage::page_size != 0) {
        throw std::runtime_error("Corrupted BPlus_tree disk file: size is not page-aligned");
    }
    ...
    if (!valid_page(_disk_header._root_page) || !valid_page(_disk_header._first_term)) {
        throw std::runtime_error("Corrupted BPlus_tree disk header: page id is outside the file");
    }
    if (_disk_header._next_page < storage::first_data_page_id) {
        throw std::runtime_error("Corrupted BPlus_tree disk header: next page is invalid");
    }
    if (_disk_header._tree_order != t || _disk_header._page_layout != disk_variable_page_layout ||
        ...
        throw std::runtime_error("Incompatible BPlus_tree disk header: tree parameters do not match");
    }

    const auto root_type = read_page_type(_disk_header._root_page);
    if (root_type != bptree_page_type::term && root_type != bptree_page_type::middle) {
        throw std::runtime_error("Corrupted BPlus_tree disk header: root page is not a tree node");
    }
    if (read_page_type(_disk_header._first_term) != bptree_page_type::term) {
        throw std::runtime_error("Corrupted BPlus_tree disk header: first term is not a term page");
    }
    if (_disk_header._next_page > pages_count) {
        throw std::runtime_error("Corrupted BPlus_tree disk header: next page is outside the file");
    }
}

template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::validate_tree() const {
    const auto file_size = std::filesystem::file_size(_pager.path());
    const auto pages_count = file_size / storage::page_size;
    ...
    if (subtree_size != _disk_header._size) {
        throw std::runtime_error("Corrupted BPlus_tree disk tree: header size does not match tree payload");
    }
    ...
    while (term_page != storage::invalid_page_id) {
        if (term_page >= pages_count || term_page == storage::header_page_id) {
            throw std::runtime_error("Corrupted BPlus_tree disk term chain: page id is outside the file");
        }
        if (term_chain_seen[term_page]) {
            throw std::runtime_error("Corrupted BPlus_tree disk term chain: cycle detected");
        }
        if (!visited_pages[term_page]) {
            throw std::runtime_error("Corrupted BPlus_tree disk term chain: term is not reachable from root");
        }

        const auto term = read_term(term_page);
        for (const auto &[key, value]: term.data) {
            if (previous_key && !compare_keys(*previous_key, key)) {
                throw std::runtime_error("Corrupted BPlus_tree disk term chain: keys are not strictly increasing");
            }
            previous_key = key;
            ++term_chain_size;
        }
    ...
    if (term_chain_size != subtree_size) {
        throw std::runtime_error("Corrupted BPlus_tree disk term chain: payload count mismatch");
    }

    auto free_page = _disk_header._first_free_page;
    while (free_page != storage::invalid_page_id) {
        if (free_page >= visited_pages.size()) {
            throw std::runtime_error("Corrupted BPlus_tree disk free list: page id is outside the file");
        }
        if (visited_pages[free_page]) {
            throw std::runtime_error("Corrupted BPlus_tree disk free list: page is already reachable");
        }
        if (read_page_type(free_page) != bptree_page_type::free) {
            throw std::runtime_error("Corrupted BPlus_tree disk free list: page is not marked free");
        }

        visited_pages[free_page] = true;
        storage::page_buffer buffer{};
        _pager.read_page(free_page, buffer);
        storage::PageReader reader(buffer, sizeof(bptree_page_type));
        free_page = reader.read_trivial<storage::page_id_t>();
    }
}

template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
std::uint64_t BPlus_tree<TKey, TValue, compare, t>::validate_subtree(
        const storage::page_id_t page_id,
    ...
    if (page_id == storage::invalid_page_id || page_id == storage::header_page_id ||
        ...
        throw std::runtime_error("Corrupted BPlus_tree disk tree: page id is outside the file");
    }
    if (reachable_pages[page_id]) {
        throw std::runtime_error("Corrupted BPlus_tree disk tree: page is referenced more than once");
    }
    reachable_pages[page_id] = true;

    const auto node_type = read_page_type(page_id);
    if (node_type == bptree_page_type::term) {
        const auto term = read_term(page_id);
        if (term._header._self != page_id || term._header._parent != expected_parent) {
            throw std::runtime_error("Corrupted BPlus_tree disk term: header page links are inconsistent");
        }
        for (std::size_t i = 1; i < term.data.size(); ++i) {
            if (!compare_keys(term.data[i - 1].first, term.data[i].first)) {
                throw std::runtime_error("Corrupted BPlus_tree disk term: keys are not strictly increasing");
            }
        }
        return term.data.size();
    }

    if (node_type != bptree_page_type::middle) {
        throw std::runtime_error("Corrupted BPlus_tree disk tree: reachable page is not a tree node");
    }

    const auto middle = read_middle(page_id);
    if (middle._header._self != page_id || middle._header._parent != expected_parent) {
        throw std::runtime_error("Corrupted BPlus_tree disk middle: header page links are inconsistent");
    }
    if (middle._pointers.size() != middle._keys.size() + 1 || middle._pointers.empty()) {
        throw std::runtime_error("Corrupted BPlus_tree disk middle: pointer/key counts are inconsistent");
    }
    if (is_root && middle._pointers.size() < 2) {
        throw std::runtime_error("Corrupted BPlus_tree disk root: middle root has too few pointers");
    }
    for (std::size_t i = 1; i < middle._keys.size(); ++i) {
        if (!compare_keys(middle._keys[i - 1], middle._keys[i])) {
            throw std::runtime_error("Corrupted BPlus_tree disk middle: separator keys are not strictly increasing");
        }
    }
    for (std::size_t i = 1; i < middle._pointers.size(); ++i) {
        if (!keys_equal(middle._keys[i - 1], subtree_first_key(middle._pointers[i]))) {
            throw std::runtime_error("Corrupted BPlus_tree disk middle: separator key does not match pointer");
        }
    }
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::write_header() {
    storage::page_buffer page{};
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
storage::page_id_t BPlus_tree<TKey, TValue, compare, t>::allocate_page() {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::deallocate_page(const storage::page_id_t page_id) {
    storage::page_buffer page{};
    storage::PageWriter writer(page);
    writer.write_trivial_many(bptree_page_type::free, _disk_header._first_free_page);
    _pager.write_page(page_id, page);
    _disk_header._first_free_page = page_id;
}

template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::release_disk_subtree(const storage::page_id_t page_id) {
    if (page_id == storage::invalid_page_id) return;
    const auto type = read_page_type(page_id);
    if (type == bptree_page_type::free) return;
    if (type == bptree_page_type::middle) {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_page_type BPlus_tree<TKey, TValue, compare, t>::read_page_type(
        const storage::page_id_t page_id) const {
    storage::page_buffer page{};
    _pager.read_page(page_id, page);
    storage::PageReader reader(page);
    return reader.read_trivial<bptree_page_type>();
}

template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_node_term BPlus_tree<TKey, TValue, compare, t>::read_term(
        const storage::page_id_t page_id) const {
    storage::page_buffer page{};
    _pager.read_page(page_id, page);
    storage::PageReader reader(page);
    ...
    node._header._type = reader.read_trivial<bptree_page_type>();
    reader.read_trivial_into(node._header._keys_count, node._header._self, node._header._parent, node._next);

    if (node._header._type != bptree_page_type::term || node._header._keys_count > maximum_keys_in_node) {
        throw std::runtime_error("Corrupted BPlus_tree disk term");
    }
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::write_term(const storage::page_id_t page_id,
                                                      const bptree_node_term &node) {
    if (node.data.size() > maximum_keys_in_node) {
        throw std::runtime_error("Disk term node is over capacity");
    }
    storage::page_buffer page{};
    storage::PageWriter writer(page);
    writer.write_trivial_many(bptree_page_type::term,
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
BPlus_tree<TKey, TValue, compare, t>::bptree_node_middle BPlus_tree<TKey, TValue, compare, t>::read_middle(
        const storage::page_id_t page_id) const {
    storage::page_buffer page{};
    _pager.read_page(page_id, page);
    storage::PageReader reader(page);
    ...
    if (node._header._type != bptree_page_type::middle || node._header._keys_count > maximum_keys_in_node) {
        throw std::runtime_error("Corrupted BPlus_tree disk middle node");
    }
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::write_middle(const storage::page_id_t page_id,
                                                        const bptree_node_middle &node) {
    ...
    storage::page_buffer page{};
    storage::PageWriter writer(page);
    writer.write_trivial_many(bptree_page_type::middle,
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
TKey BPlus_tree<TKey, TValue, compare, t>::subtree_first_key(const storage::page_id_t page_id) const {
    if (read_page_type(page_id) == bptree_page_type::term) {
        const auto term = read_term(page_id);
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::set_node_parent(const storage::page_id_t page_id,
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::rebuild_separator_keys(const storage::page_id_t page_id) {
    if (read_page_type(page_id) == bptree_page_type::term) return;
    auto middle = read_middle(page_id);
    for (const auto pointer: middle._pointers) {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::rebuild_middle_keys(bptree_node_middle &node) {
    node._keys.clear();
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::rebalance_term(const storage::page_id_t term_page,
                                                          bptree_node_term &&term) {
    ...
    if (pointer_it == parent._pointers.end()) throw std::runtime_error("Corrupted BPlus_tree disk term parent");
    const auto pointer_index = pointer_it - parent._pointers.begin();


    if (pointer_index > 0 && read_page_type(parent._pointers[pointer_index - 1]) == bptree_page_type::term) {
        if (auto left = read_term(parent._pointers[pointer_index - 1]);
            left.data.size() > minimum_keys_in_node) {
    ...
    if (pointer_index + 1 < parent._pointers.size() &&
        read_page_type(parent._pointers[pointer_index + 1]) == bptree_page_type::term) {
        if (auto right = read_term(parent._pointers[pointer_index + 1]);
            right.data.size() > minimum_keys_in_node) {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::rebalance_middle(
    ...
    if (pointer_it == parent._pointers.end()) throw std::runtime_error("Corrupted BPlus_tree disk middle parent");
    const auto pointer_index = pointer_it - parent._pointers.begin();


    if (pointer_index > 0 && read_page_type(parent._pointers[pointer_index - 1]) == bptree_page_type::middle) {
        if (auto left = read_middle(parent._pointers[pointer_index - 1]);
            left._pointers.size() > minimum_pointers_in_node) {
    ...
    if (pointer_index + 1 < parent._pointers.size() &&
        read_page_type(parent._pointers[pointer_index + 1]) == bptree_page_type::middle) {
        if (auto right = read_middle(parent._pointers[pointer_index + 1]);
            right._pointers.size() > minimum_pointers_in_node) {
    ...
    if (pointer_index > 0 && read_page_type(parent._pointers[pointer_index - 1]) == bptree_page_type::middle) {
        ...
        const auto separator_key = parent._keys[pointer_index - 1];

        left._keys.push_back(separator_key);
        left._keys.insert(left._keys.end(), node._keys.begin(), node._keys.end());
        left._pointers.insert(left._pointers.end(), node._pointers.begin(), node._pointers.end());
        ...
        parent._pointers.erase(parent._pointers.begin() + pointer_index);
        parent._keys.erase(parent._keys.begin() + pointer_index - 1);
    ...
    if (pointer_index + 1 < parent._pointers.size() &&
        ...
        const auto separator_key = parent._keys[pointer_index];

        node._keys.push_back(separator_key);
        node._keys.insert(node._keys.end(), right._keys.begin(), right._keys.end());
        node._pointers.insert(node._pointers.end(), right._pointers.begin(), right._pointers.end());
        ...
        parent._pointers.erase(parent._pointers.begin() + pointer_index + 1);
        parent._keys.erase(parent._keys.begin() + pointer_index);
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::split_root_term(bptree_node_term &&term) {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::split_term_node(const storage::page_id_t term_page,
                                                           bptree_node_term &&term) {
    ...
    auto parent = read_middle(term._header._parent);
    const auto pointer_it = std::ranges::find(parent._pointers, term_page);
    if (pointer_it == parent._pointers.end()) throw std::runtime_error("Corrupted BPlus_tree disk parent");
    const auto pointer_index = pointer_it - parent._pointers.begin();


    if (pointer_index > 0 && read_page_type(parent._pointers[pointer_index - 1]) == bptree_page_type::term) {
        if (auto left = read_term(parent._pointers[pointer_index - 1]);
            left.data.size() < maximum_keys_in_node) {
            distribute_term_pair(left, term);
            write_term(left._header._self, left);
            write_term(term_page, term);
            wrebuilt_middle(parent);
    ...
    if (pointer_index + 1 < parent._pointers.size() &&
        read_page_type(parent._pointers[pointer_index + 1]) == bptree_page_type::term) {
        if (auto right = read_term(parent._pointers[pointer_index + 1]);
            right.data.size() < maximum_keys_in_node) {
            distribute_term_pair(term, right);
            write_term(term_page, term);
            write_term(right._header._self, right);
            wrebuilt_middle(parent);
    ...
    auto new_term = make_term(new_page, parent._header._self, term._next);
    new_term.data.assign(std::make_move_iterator(term.data.begin() + split_at), std::make_move_iterator(term.data.end()));
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::split_root_middle(bptree_node_middle &&root) {
...
template<typename TKey, typename TValue, comparator<TKey> compare, std::size_t t>
void BPlus_tree<TKey, TValue, compare, t>::split_middle_node(const storage::page_id_t middle_page,
                                                             bptree_node_middle &&node) {
    ...
    if (pointer_index > 0 && read_page_type(parent._pointers[pointer_index - 1]) == bptree_page_type::middle) {
        if (auto left = read_middle(parent._pointers[pointer_index - 1]);
            left._pointers.size() < maximum_pointers_in_node) {
            distribute_middle_pair(left, node);
            write_middle(left._header._self, left);
            write_middle(middle_page, node);
            wrebuilt_middle(parent);
    ...
    if (pointer_index + 1 < parent._pointers.size() &&
        read_page_type(parent._pointers[pointer_index + 1]) == bptree_page_type::middle) {
        if (auto right = read_middle(parent._pointers[pointer_index + 1]);
            right._pointers.size() < maximum_pointers_in_node) {
            distribute_middle_pair(node, right);
            write_middle(middle_page, node);
            write_middle(right._header._self, right);
            wrebuilt_middle(parent);
    ...
    const auto new_page = allocate_page();
    const auto split_at = node._pointers.size() / 2;

    auto new_middle = make_middle(new_page, parent._header._self);
    new_middle._pointers.assign(node._pointers.begin() + split_at, node._pointers.end());
    new_middle._keys.assign(node._keys.begin() + split_at, node._keys.end());

    const auto promoted_key = node._keys[split_at - 1];

    node._pointers.resize(split_at);
    node._keys.resize(split_at - 1);

    parent._pointers.insert(parent._pointers.begin() + pointer_index + 1, new_page);
    parent._keys.insert(parent._keys.begin() + pointer_index, promoted_key);

    write_middle(middle_page, node);
    write_middle(new_page, new_middle);

    for (const auto pointer: new_middle._pointers) {
        set_node_parent(pointer, new_page);
    }
